  ...
  try {
    for (let i = 0; i < count; i++) {
      ...
      const id = await pubsub().topic(fake ? FAKE_TOPIC : def.model).publishMessage({
        json: { jobId, step, unit: i, attempt, type: "step", model: def.model, subtype: def.subtype, tools: def.tools || [], ...(fake ? { fake: true, item: Array.isArray(def.items) ? def.items[i] : null, ctx: { days: def.renderCtx?.days, meals: def.renderCtx?.meals, proteins: def.renderCtx?.proteins } } : {}), ...(def.style ? { style: def.style } : {}), ...(report ? { report } : {}), ...(unitQuery != null ? { query: unitQuery } : {}) },
      });
      msgIds.push(id);
    }
  } catch (err) {
