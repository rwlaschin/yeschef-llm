...
//
//   node scripts/devbox.js list                 every box: state, URL, machine, cost
//   node scripts/devbox.js create  <box> [opts] build the VM wherever L4 capacity exists
//   node scripts/devbox.js status  <box>        one box in detail
//   node scripts/devbox.js start   <box>        build + wait for Ollama
//   node scripts/devbox.js stop    <box>        DELETE the VM
//   node scripts/devbox.js stop-all             DELETE every RUNNING VM
//   node scripts/devbox.js allow                add the network you're on to the allowlist
//   node scripts/devbox.js allowlist            who can reach the boxes right now
//   node scripts/devbox.js pull    <box> <model>
//   node scripts/devbox.js delete  <box>        remove the VM (its IP is ephemeral and goes with it)
//
...
import { L4_ZONES, ALL_BOX_NAMES, boxDefaults, vmOf, hostOf, rateFor } from "../config/devboxes.js";
import { devModels, parallelOf } from "../config/models.js";
...
const STOCKOUT = /ZONE_RESOURCE_POOL_EXHAUSTED/;
...
const IMAGE = "--image-family=common-cu129-ubuntu-2204-nvidia-580 --image-project=deeplearning-platform-release";

export function buildStartupScript({ timeoutMinutes = 0, parallel = BOX_PARALLEL } = {}) {
  const lines = [
    "#!/bin/bash",
    "set -e",
...
const STARTUP_FILE = process.env.DEVBOX_STARTUP_FILE || `${process.env.HOME}/.yeschef-devbox-startup.json`;

function readStartupFile() {
  try { return JSON.parse(readFileSync(STARTUP_FILE, "utf8")); } catch { return {}; }
}

export function readStartupState(name) {
  const s = readStartupFile()[name];
  if (!s) return null;
  // A writer that died leaves a stale entry; treat anything unrefreshed for 3 min as gone.
  return Date.now() - (s.timestamp || 0) > 180000 ? null : s;
}

export function clearStartupState(name) {
  const all = readStartupFile();
  delete all[name];
  try { writeFileSync(STARTUP_FILE, JSON.stringify(all, null, 2)); } catch {}
...
export function cancelStartupState(name) {
  const all = readStartupFile();
  all[name] = { ...(all[name] || {}), cancelled: true, timestamp: Date.now() };
  try { writeFileSync(STARTUP_FILE, JSON.stringify(all, null, 2)); } catch {}
  const active = startupTracker.get(name) || {};
  startupTracker.set(name, { ...active, cancelled: true });
}
...
export function endStartupState(name, phase, msg) {
  const all = readStartupFile();
  all[name] = { phase, msg, timestamp: Date.now() };
  try { writeFileSync(STARTUP_FILE, JSON.stringify(all, null, 2)); } catch {}
  startupTracker.delete(name);
}

function updateStartupState(name, data) {
  const prev = startupTracker.get(name) || readStartupState(name);
  if (prev?.cancelled || readStartupState(name)?.cancelled) return;
...
const findCapacity = (b, { sh = defaultSh, timeoutMinutes = 0, machineOpt, gpusOpt, diskOpt, roundsOpt } = {}) => {
  ...
  while (r <= maxRounds) {
    for (let idx = 0; idx < zones.length; idx++) {
      ...
      const existing = allBoxes(sh).find((x) => x.vm === b.vm);
      if (existing) {
        console.log(`  ${existing.zone}: already exists (${existing.status}) — search over.`);
        updateStartupState(b.name, { phase: "created", zone: existing.zone, stockouts, msg: `Allocated in ${existing.zone} · Initializing OS & network…` });
        return existing.zone;
      }
      // Via a real file, never a heredoc on /dev/stdin: the startup script contains its OWN `EOF`
      // (the idle-watchdog heredoc), which closed the outer one early and silently truncated the
      // script at that line — the watchdog file got written but `chmod +x` and the `nohup` that
      // actually starts it were cut off, so a box with --timeout never shut itself down.
      const startupFile = `${tmpdir()}/${b.vm}-startup.sh`;
      writeFileSync(startupFile, startup);
      const out = sh(`gcloud compute instances create ${b.vm} ${P} --zone=${zone} ` +
        `--machine-type=${machine} --accelerator=type=nvidia-l4,count=${gpus} --maintenance-policy=TERMINATE ` +
        `${IMAGE} --boot-disk-size=${disk}GB --boot-disk-type=pd-balanced ` +
        // cloud-platform scope so the idle watchdog can delete its own instance; the default scopes
        // omit compute entirely, which would make that call 403 and fall back to a billing disk.
        `--tags=${TAG} --labels=owner=dev,purpose=ollama-devbox ` +
        `--scopes=https://www.googleapis.com/auth/cloud-platform ` +
        `--metadata-from-file=startup-script=${startupFile}`, true, 300000);
      if (startupTracker.get(b.name)?.cancelled || readStartupState(b.name)?.cancelled) {
        console.log(`Capacity search for ${b.name} cancelled by user.`);
        startupTracker.delete(b.name);
        return "";
      ...
      if (/Reauthentication|cannot prompt|not authenticated|credentials/i.test(out)) {
        console.error(`Auth token expired during capacity probe.`);
        startupTracker.delete(b.name);
        throw new Error(`GCP authentication expired. Please run 'gcloud auth login' in your terminal.`);
      }
      ...
      if (!STOCKOUT.test(out)) {
        const live = sh(`gcloud compute instances describe ${b.vm} ${P} --zone=${zone} --format="value(status)" 2>&1`, true, 60000);
        if (live && !/ERROR|was not found/i.test(live)) {
          console.log(`  ${zone}: CREATED (recovered — create reported "${out.slice(0, 60)}" but the VM exists)`);
          updateStartupState(b.name, { phase: "created", zone, stockouts, msg: `Allocated in ${zone} · Initializing OS & network…` });
          return zone;
        }
      }
      if (STOCKOUT.test(out)) stockouts++;
      console.log(STOCKOUT.test(out) ? `  ${zone}: stockout` : `  ${zone}: ${(out.match(/ERROR:.*/) || ["error"])[0]}`);
      ...
      updateStartupState(b.name, {
        ...
        msg: STOCKOUT.test(out) ? `${zone}: stockout — switching region…` : `${zone}: error — switching region…`,
      });
      sh(`sleep 5`, true);
    }
    r++;
...
const waitForOllama = (url, secs = 600, name = "", sh = defaultSh) => {
  process.stdout.write(`Waiting for ${url}`);
  let code = 1;
  for (let i = 0; i < secs / 5; i++) {
    code = probe(url, sh);
    if (code === 0) { console.log(` — up.`); return 0; }
    if (name) updateStartupState(name, { phase: "installing", msg: `Booting VM & installing Ollama…` });
...
const needName = (verb, sh = defaultSh) => {
  ...
  if (!name || name.startsWith("--")) {
    console.error(`usage: node scripts/devbox.js ${verb} <box>\nBoxes: ${allBoxes(sh).map((b) => b.name).join(", ") || "(none)"}`);
    process.exit(1);
  }
  return box(name, sh);
};

export const syncHosts = ({ sh = defaultSh } = {}) => {
  const boxes = allBoxes(sh).filter((b) => b.ip);
  if (!boxes.length) {
...
export async function getFleetStatus({ exec = defaultSh, clientIp = "" } = {}) {
  ...
  const boxes = declaredNames.map((name) => {
    ...
    if (usable) clearStartupState(name); // a box with no model has not finished starting
...
const cliStartOpts = () => ({
  timeoutMinutes: Number(flag("timeout", "0")) || 0,
...
export async function startDevbox(name, { exec = defaultSh, timeoutMinutes = 0, model, machine, gpus, disk, rounds } = {}) {
  try {
    const auth = checkAuth(exec);
    if (!auth.ok) {
      startupTracker.delete(name);
    ...
    const s = describe(b, "status", exec);
    if (s === "RUNNING") {
      console.log(`${b.vm} already RUNNING in ${b.zone}.`);
      syncHosts({ sh: exec });
      waitForOllama(urlOf(b, exec), 600, name, exec);
      startupTracker.delete(name);
      return { ok: true, status: "RUNNING", message: `${b.vm} already RUNNING in ${b.zone}.` };
    }
    if (s && s !== "TERMINATED") {
      fail(`${b.vm} exists in ${b.zone} with status ${s} — not adopting a half-state. Delete it, then start again.`);
    }
    if (s === "TERMINATED") {
      exec(`gcloud compute instances delete ${b.vm} ${b.G} --quiet`, true, 300000);
    }

    ensureFirewall({ sh: exec });
    clearStartupState(name); // a cancelled flag from a previous stop must not gag this run's progress
    updateStartupState(name, {
      phase: "hunting",
      zone: L4_ZONES[0],
      round: 1,
      stockouts: 0,
    ...
    const zone = findCapacity(b, { sh: exec, timeoutMinutes, machineOpt: machine, gpusOpt: gpus, diskOpt: disk, roundsOpt: rounds });
    if (!zone) {
      const wasCancelled = readStartupState(name)?.cancelled
      if (!wasCancelled) endStartupState(name, "failed", `No L4 capacity available — search exhausted every US zone.`);
    ...
    const ownCreatedAt = allBoxes(exec).find((x) => x.vm === boxInst.vm)?.createdAt || "";
    const stillOurs = () => {
      const cur = allBoxes(exec).find((x) => x.vm === boxInst.vm);
      return !!cur && (!ownCreatedAt || cur.createdAt === ownCreatedAt);
    };

    updateStartupState(name, { phase: "installing", zone, msg: `Booting VM in ${zone} · Installing Ollama daemon…` });
    const code = waitForOllama(`http://${ip}:${PORT}`, 600, name, exec);
    ...
    updateStartupState(name, { phase: "pulling", zone, msg: `Pulling ${targetModel} weights into GPU VRAM…` });
    // A multi-GB pull takes minutes; on the 8s default this returns ETIMEDOUT and reports a
    // false "model NOT pulled" for a pull that actually succeeded.
    const pull = exec(`gcloud compute ssh ${boxInst.vm} ${boxInst.G} --tunnel-through-iap --command="ollama pull ${targetModel}"`, true, 1800000);
    if (/^ERROR/.test(pull)) {
      startupTracker.delete(name);
      fail(`\n${targetModel} was NOT pulled onto ${b.vm}. The box is RUNNING and BILLING, and unusable until it has a model.\n` +
           `${pull.slice(0, 500)}\nRetry:  node scripts/devbox.js pull ${b.name} ${targetModel}`);
    }
    ...
    endStartupState(name, "ready", `${targetModel} ready in ${zone} — box is RUNNING.`);
    return { ok: true, status: "RUNNING", zone, ip, model: targetModel };
  } catch (err) {
    const msg = err.message || String(err);
    endStartupState(name, "failed", msg.slice(0, 200));
    if (/Reauthentication|auth tokens|cannot prompt|gcloud auth login/i.test(msg)) {
      return { ok: false, authRequired: true, error: "GCP authentication expired. Please run 'gcloud auth login' in your terminal.", message: `BLOCKED: GCP authentication expired. Please run 'gcloud auth login'.` };
    }
...
export async function stopDevbox(name, { exec = defaultSh } = {}) {
  ...
  if (!allBoxes(exec).length && sources(exec).length) {
    exec(`gcloud compute firewall-rules delete ${FW} ${P} --quiet`, false, 120000);
    console.log(`Last box gone — firewall rule deleted.`);
  }
  startupTracker.delete(name);
...
export const cmds = {
  ...
  async create() {
    ...
    await startDevbox(b.name, cliStartOpts());
  },
  async start() {
  ...
  async pull() {
    ...
    if (!model) return fail(`usage: node scripts/devbox.js pull <box> <model>`);
  ...
  async models() {
    ...
    if (/ERROR/.test(tags) || !tags) return fail(`${b.vm} is not answering at ${url}. Start it, or run: node scripts/devbox.js allow`);
    ...
    console.log(`\n● = loaded in VRAM now.  Switch with: node scripts/devbox.js use ${b.name} <model>`);
  },
  async use() {
    const b = needName("use");
    const model = process.argv[4];
    if (!model) return fail(`usage: node scripts/devbox.js use <box> <model>`);
  ...
  async chat() {
    ...
    if (!model) return fail(`${b.name} holds no models. node scripts/devbox.js use ${b.name} <model>`);
  ...
  async delete() {
    ...
    if (!allBoxes().length && sources().length) { defaultSh(`gcloud compute firewall-rules delete ${FW} ${P} --quiet`, false, 120000); console.log(`Last box gone — firewall rule deleted.`); }
  },
};
...
if (isCli) {
  ...
  if (!cmds[cmd]) {
    console.log(`usage: node scripts/devbox.js <list|create|start|stop|stop-all|status|allow|allowlist|models|use|chat|pull|hosts|delete> [box]\n`);
