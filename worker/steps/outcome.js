...
//
//   PASS → @@::PASS::@@ or @@::PASS;:&@
//   FAIL → @@::FAIL:<REASON>::@@ or @@::FAIL:<REASON>;:&@
//
//   • REQUIRED DELIMITERS: the literal "@@::" at the start and either supported close at the end. NO angle brackets —
//     weak models drop "<"/">" ("<" is special-token territory to the tokenizer) and HTML/markdown
//     renderers eat "<…>" as a tag; the 8B reproduces these bare delimiters verbatim.
...
//   • We dropped developit's "?!"/"!?" and "PLAN_STATUS" word; the status bookends are
...
// Full block (final parse), accepting the legacy "::@@" and current ";:&@" closers. PASS stands alone;
...
// outcome). splitOutcome surfaces such a reasonless fail instead. The closing token terminates
...
// the block has begun, so the visible response freezes here even before its close streams in.
...
export function splitOutcome(full) {
  ...
  const m = s.match(MARKER); // well-formed block, with either supported closing token
