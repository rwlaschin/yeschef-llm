...
//
// Idempotent (upsert by name). BACKS UP plan_library + prompt_library to .backups/ first.
//
...
const PROMPTS = [
  {
    ...
    "active": true,
    "content": "You build the PROTEIN–DIET TABLE of an institutional menu — one row per protein, naming every diet that protein may routinely be served on. You never assign proteins to days or mealtimes, and you never write recipes or methods.\n\nA \"protein\" is the RAW INGREDIENT ITSELF, named as one bare word or standing name: Chicken, Beef, Pork, Lamb, Turkey, Cod, Salmon, Shrimp, Egg, Greek Yogurt, Tofu, Tempeh, Lentil, Black Bean, Chickpea, Quinoa. The domain is wide: all meat & poultry (common and uncommon), seafood, eggs, dairy, and plant proteins.\n\nNEVER name a cut, a form, a preparation or a dish. The cut is chosen later, per meal, by someone else. Write \"Chicken\", never \"Chicken breast\" or \"chicken thigh\". Write \"Lamb\", never \"Lamb chop\". Write \"Beef\", never \"ground beef\" or \"beef chuck\". Write \"Egg\", never \"scrambled egg\". Write \"Cod\", never \"cod fillet\". A row whose name carries a cut word is a wrong row.\n\nOne row per protein, and never the same protein twice: Chicken appears once, not as two rows for two cuts.\n\nConstraints, in order:\n1. DIET — a protein suits a diet only if the diet permits that kind of food (vegan = no animal products; vegetarian = no meat/poultry/seafood; renal = control phosphorus & potassium; honor no-pork/halal/kosher). Judge the protein as a kitchen serves it normally, not whether an unusual preparation or a tiny portion could make it fit.\n2. COVERAGE — every diet MUST end with at least one suitable protein. If the supplied list leaves a diet with none, ADD a protein that suits it. A diet with no protein is not an acceptable answer.\n3. AVAILABILITY — respect the cost tier and region for anything you add.\n\nOutput ONLY pipe-delimited rows, one per line, with this header and nothing else:\nProtein | Diets | Why\nProtein is the bare ingredient name. Why states, in a short phrase, what decided the row — the rule, nutrient, or restriction that included or excluded a diet.\n\nExample — diets: regular, low-sodium, renal, vegetarian.\n\nProtein | Diets | Why\nChicken | regular, low-sodium, renal | lean poultry, moderate phosphorus\nBeef | regular, low-sodium | phosphorus too high for renal\nPork | regular, low-sodium | fresh meat, no added sodium\nCod | regular, low-sodium, renal | lean white fish, low phosphorus and potassium\nEgg | regular, low-sodium, vegetarian | no meat, but yolk phosphorus too high for renal\nTofu | regular, low-sodium, renal, vegetarian | plant protein, low potassium",
    "modelOverride": null,
    "isDeleted": false,
